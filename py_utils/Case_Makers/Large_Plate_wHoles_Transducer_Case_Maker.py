...
def main(argv):
    ...
    plate_dims = [208e-3, 144.0e-3, 0.81e-3] #m
    ds = [2.5e-4, 2.5e-4, 5.0e-6] #m

    x_cut = 50e-3

    y_offset = 24.24e-3
    x_offset = 95.12e-3 - x_cut #128.65e-3 - 9.65e-3 - 23.88e-3 
    y_spacing = 23.88e-3
    x_spacing = 15.25e-3
    ...
    fname = 'large_AL_plate_w_holes_Transducer_Example.vtu'
...
def makePlateSim(fname, plate_dim, ds, hole_locs, hole_rads, tVec, wo_arr):
    ...
    # # Smooth Pulse
    # TiSpeed = 6313 #m/s Titanium 
    # ToF = 2*block_dim[2]/TiSpeed
    # pulseWidth = 0.2 *ToF 
    # wf_Tend = np.argmin(np.abs(tVec-pulsWidth))
    ...
    array = numpy_to_vtk(np.array([9]), deep=True)
    name = "NTransducers"
    array.SetName(name)
    ugrid.GetFieldData().AddArray(array)
