...
def makeAngleBracket(bracket_geometry, material_properties, timing, transducer, crack_properties, filename):
    ...
    geometry[1:-1,1:1+thickness_cells_y,1:-1] = 1
    ...
    # ## Add Crack
    c_length = int(crack_properties["length"]/ds[0])
    c_width = int(crack_properties["width"]/ds[1])
    c_x_loc = 1+int(dimensions[0]/2/ds[0])
    c_z_loc = 1+thickness_cells_y
    geometry[c_x_loc:c_x_loc+c_length, :, c_z_loc:c_z_loc+c_width]=0
    ...
    c_width = int(crack_properties["width"]/ds[1])
    c_x_loc = 1+int(dimensions[0]/2/ds[0])
    c_y_loc = 1+thickness_cells_y
    geometry[c_x_loc:c_x_loc+c_length,c_y_loc:c_y_loc+c_width,:]=0
    print("Total Cells = %d" %(np.count_nonzero(geometry)))
...
def main(args):
    ...
    bracket_geometry = {
        "bracket_length": 7.5e-2, 
        "bracket_height": 2.0e-2, 
        "bracket_thickness": 2.0e-3, 
        "cell_dimensions": np.array([40e-06, 40e-06, 40e-06])}

    material_properties = {
        "density": 2780, 
        "lame_1": 51.74993908e9,
        "lame_2":26.664117020e9,}

    timing = {
        "dt": 2.0e-9, 
        "write_interval": 500, 
        "simulation_duration": 60e-6}
    ...
    transducer =  {
        ...
        "cycle_count": 3, 
        "phase": 0, 
        "location": np.array([int(args[1])*1.0e-6, (bracket_geometry["bracket_height"]-bracket_geometry["bracket_thickness"])/2, 0.0])} #center of the flange 

    print(transducer["location"])

    crack_properties = {
        "width": 150e-6,
        "length": int(args[0])*1.0e-6
    }

    filename = "AL_Angle_Bracket_Crack_%sum_%sum.vtu" % (args[0], args[1])
    
    makeAngleBracket(bracket_geometry, material_properties, timing, transducer, crack_properties, filename)
    return


if __name__ == '__main__':

    # Arg 1: Crack Length in um
    # Arg 2: X position of transducer in um
